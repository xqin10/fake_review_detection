...
def results():
    ...
    if request.method == 'POST':
      ...
      urlinput_pagination = request.form['urlinput_pagination']
...
@app.route('/results/api/v1',methods=['POST'])
def results_api():
    """
    Do prediction with one url from pront-end.
    """
    if request.method == 'POST':
      ...
      # data = []
      # try:
      #   biz_type, platform_type, scrape_data, _ = scrape(url)
      #   data = [d['review'] for d in scrape_data]
      # except BaseException:
      #   print("Scrape timeout!")
    ...
    return jsonify({
                    ...
                    'fake_reviews_count': fake_reviews_count,
                    'true_reviews_count': true_reviews_count,
                    'total_reviews_count': fake_reviews_count+true_reviews_count
                    })
